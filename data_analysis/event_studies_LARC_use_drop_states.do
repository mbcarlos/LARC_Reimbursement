...
cd "${S_DATE}"
log using "${log_path}/event_studies_LARC_use_drop_states${S_DATE}.log", replace text
...
local state_list CT FL ID LA MA ME OK RI VT WV // this is the list of state that have separate device reimbursement in the LARC data
...
foreach state of local state_list {
	...
	*reg log_`outcome_var' t_* i.`time_period'##i.year i.state_num medicaid_expanded  if separate_device_reimb_indata==1, noomitted cluster(state_num)
	...
	reg log_larc_count t_* i.quarter#i.year i.state_num medicaid_expanded unemployment_rate_1yrlag if separate_device_reimb_indata==1 & state!="`state'", noomitted cluster(state_num)
	* end 8 **************************************************************************************
	...
	** Create variables from matrix
	cap drop tdiff beta upperlimit lowerlimit
	...
	** summarize number of states for graph notes: 
	...
	qui egen tag_states_sep_reimb = group(state_short) if separate_device_reimb_indata==1 & state!="`state'"
	...
	twoway ///
		...
		title("LARC Use in Medicaid (`state' dropped)") ///
		note("Notes: " ///
	...
	****NOTE: If adding more notes to graph make sure to add them BEFORE "`merge_notes'" becuase merge_notes local is empty for month datasets which cutoffs comments after
	graph export larc_utilization_quarterly_ES_no`state'.pdf, replace
	* end 10 *************************************************************************************
}
